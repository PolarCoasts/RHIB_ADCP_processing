function f_interp = gps_line_interp(gps,dn_source,prefix,field,valtype)

arguments
    gps                                                                 % structure with gps data
    dn_source                                                           % NMEA prefix for date/time field (e.g. 'GPRMC')
    prefix                                                              % NMEA prefix field to be interpolated (e.g. 'HEHDT')
    field                                                               % field to be interpolated (e.g. 'head')
    valtype char {mustBeMember(valtype,{'linear','angular'})}='linear'  % specify whether desired data is linear or angular (periodic)
end

% Interpolate a field from an NMEA prefix without date/time information to date/time from a different NMEA prefix. 
% For example, interpolate heading from $HEHDT lines to timestamps from $GPRMC lines.
%
...
for i = 1:length(gps.files)
    ...
    for ip = 1:length(prefixes)
        ...
        lnum_max = max([lnum_max; max(gps.(prefixes{ip}).lnum(idx))],[],'omitnan');
    end
    for ip = 1:length(prefixes)
...
if ~isempty(idx)
    % This is the cool part... Using some sparse matrix magic, efficiently find the preceeding timestamp line for each line containing our desired field.
    ...
    s = s(:,s(1,:)<=length(gps.(dn_source).lnum) & s(2,:)<=length(gps.(prefix).lnum));

    dn = gps.(dn_source).dn(s(1,:));
    f = gps.(prefix).(field)(s(2,:));
    [~,uidx] = unique(dn);
    if strcmp(valtype,'angular')
        f_interp = mod(180/pi*angle(interp1(dn(uidx),cosd(f(uidx))+1i*sind(f(uidx)),gps.(dn_source).dn)),360);
    elseif strcmp(valtype,'linear')
