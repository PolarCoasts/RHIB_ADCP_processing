...
% Inputs
% - f_in
%   This can be a filename, cell array of filenames, the output of MATLAB's "dir" command.
%
...
% Outputs
% - gps
%   Data structure containing GPS fields. Each type of NMEA message gets its own sub-structure.
%
% Orignal author: Dylan Winters

function GPS = parse_gps(f_in)
...
% The portions enclosed in parentheses will be extracted, while the rest is just used for matching. 
% The regexp ',?' means that there might be a comma.
...
% Combine the above regexps for single chunks of data into regexps for different types of complete NMEA strings:
fmt = struct();
fmt.HEHDT = ['\$HEHDT,' dec];
fmt.HEROT = ['\$HEROT,' dec];
fmt.GPGGA = ['\$GPGGA,' ...
             ...
             intu intu         ... % qual, #sats
             dec dec           ... % hdop, alt
             ltr dec ltr       ... % alt units, undulation, und units
             dec];                 % aod
fmt.GPHDT = ['\$GPHDT,' dec];
fmt.GPHEV = ['\$GPHEV,' dec];
fmt.GPRMC = ['\$GPRMC,' ...
...
% add a field for nuc timestamps
fmt.nuc_time = ['\n' dec '\n'];
...
filts = struct('GPRMC', ewflt,'GPGGA', ewflt);

%% Function handles for extracting fields
% Each file is consecutively parsed for data from each NMEA type. All lines of a single NMEA type are extracted 
% at once, into a matrix D with a row for each line and a column for each raw field. The function handles below 
% provide instructions for converting this matrix into meaningful data.
%
% Defining this structure in this way allows for easy looping through NMEA prefixes and fields within each prefix.
...
flds = struct(...
    ...
    'HEROT',struct(...
        'rot',   @(D) D(:,1))                                ,...
    'GPRMC',struct(...
        'dn',    @(D) datenum(D(:,[13 12 11 1 2 3])) + datenum([2000 0 0 0 0 0])              ,...
        'lat',   @(D) D(:,4) + D(:,5)/60                     ,...
        'lon',   @(D) D(:,8).*(D(:,6) + D(:,7)/60)           ,...
    ...
    'nuc_time',struct( ...
        'dn', @(D) datenum(string(abs(D)),'yyyymmddHHMMSS')));
...
for fi = 1:length(f_in)
    ...
    for i = 1:length(nmea_types)
        ...
        if ~isempty(lines)
            ...
            vars = fields(flds.(prefix));
            % Grab line numbers by counting occurences of newline characters before the start of each line:
            lnum = nan(size(D,1),1);
            lnum(1) = 1 + length(regexp(ftxt(1:start(1)),'\n'));
            for l = 2:length(lnum)
                lnum(l) = lnum(l-1) + length(regexp(ftxt(start(l-1):start(l)),'\n'));
            end
            ...
            for v = 1:length(vars)
                GPS.(prefix).(vars{v}) = cat(1,GPS.(prefix).(vars{v}),flds.(prefix).(vars{v})(D));
            end
        end
    end
end
