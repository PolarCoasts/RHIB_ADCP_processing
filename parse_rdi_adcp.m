...
% Inputs
% - dat_or_file
%   This can be a filename, cell array of filenames, the output of MATLAB's "dir" command, or an array of binary data.
%
...
function adcp = parse_rdi_adcp(dat_or_file,parse_nuc_timestamps)

arguments
    dat_or_file
    parse_nuc_timestamps logical=false
end
    ...
    [h,len] = find_headers(dat); % find header locations
    if isempty(h)
        adcp = []; return
    end

    %% Fill adcp data structure with ensemble data
    % Loop over unique ensemble lengths (there coule be multiple, e.g. for Sentinel V's multi-profile mode)
    [u,~,iu] = unique(len);
    for nl = 1:length(u)
    ...
    adcp.processing.bt_removed_from_vel = false;
    ...
    function [h,len] = find_headers(dat)
    ...
    % Initialize output structure, read number of data types and offsets from first header. If we're parsing bytes inserted prior 
    % to each header, account for the number of bytes inserted.
        adcp = struct();
        ...
        for i = 1:nfields
            % For most fields, we can use the following function to extract data columns. It looks nasty, but it's just doing the following:
            ...
            switch dec2hex(typecast(dat(1,offsets(i) + [1:2]),'uint16'),4)
              case '0000' % fixed leader
                ...
                % The fixed leader is in every ensemble, but we only need to parse 1. 
                % In this case, modify the getdat function to only get data from the first row.
                ...
                adcp.cell_depth = adcp.config.bin_1_distance + (0:adcp.config.n_cells-1)'*adcp.config.depth_cell_length;

                % The ROSE computers insert timestamps outside of the ADCP's data structure. These don't have an ID or offset, so just grab
                % them along with the fixed leader. Parse inserted bytes after reading fixed leader.
                if parse_nuc_timestamps
              ...
              case '0080' % variable leader
                ...
                adcp.time = datenum([100*clock_century+clock_year, clock_month, clock_day, clock_hour, clock_minute, clock_second + clock_hundr/100])';

              % These data need some additional reshaping since they have beam, depth, and time dimensions.
              case '0100' % velocity
              ...
              % Bottom-track data
              case '0600'
                % BT range is a 3-byte integer with the most significant byte much later in the data packet.
